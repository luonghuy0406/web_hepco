...
import ShareholderNewest from './ShareholderNewest'
        ...
        function doneTyping () {
            if(didMount.current){
                ...
                .then(result => {
                    ...
                    setTotalPages(Math.ceil(data.num_shareholder/itemsPerPage))
                ...
                <Grid item container spacing={3} xs={12} md={9}>
                    ...
                    </Grid>
                    {
                        ...
                        currentPageData.map((shareholder) => {
                            const normalizedTitle = (shareholder["name_"+currentLang] || shareholder.name).toLowerCase().replace(/\s+/g, '-').replace(/[^\w-]/g, '')
                            return(
                                <Grid key={shareholder.id_shareholder} item xs={12} sm={6} md={4}>
                                    <Link to={`/${t('codong')}/${normalizedTitle}.${shareholder.id_shareholder}`}>
                                        <FeaturedShareholdertWrap>
                                            <LazyLoad height={200} offset={100}>
                                            <FeaturedShareholdertImage image={`${process.env.REACT_APP_HOST}/read_image/${shareholder.image}`}>
                                            {/* <FeaturedShareholdertImage image={`https://t4.ftcdn.net/jpg/04/73/25/49/360_F_473254957_bxG9yf4ly7OBO5I0O5KABlN930GwaMQz.jpg`}> */}
                                                ...
                                                <Typography 
                                                    ...
                                                    fontWeight={700}
                                                >{formatDateTime(shareholder.cre_date,'%d-%m-%Y')}</Typography>
                                                </Box>
                                            ...
                                            <Grid container spacing={1}>
                                                <Grid item xs={12} pb={1}>
                                                    <Typography variant="h6" color={theme.color.red} fontWeight={700}>{shareholder["name_"+currentLang] || shareholder.name}</Typography>
                                                </Grid>
                                                <Grid item xs={12}>
                                                    <Box sx={{display:'flex'}}>
                                                    ...
                                                    <Typography color={theme.color.gray1} fontWeight={700} >{categories?.[shareholder.type_id]?.name}</Typography>
                                                    </Box>
                                                </Grid>
                                                <Grid item xs={12}>
                                                    <Box className='ck-content' dangerouslySetInnerHTML={{__html:shareholder["content_"+currentLang] || shareholder.content}} sx={{overflow: 'hidden', display: '-webkit-box', WebkitLineClamp: 3, WebkitBoxOrient: 'vertical'}}>
                                                    </Box>
                                                </Grid>
                                                {/* <Grid item>
                                                    <BlogButton>
