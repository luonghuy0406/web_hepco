...
export default function RelatedProjects({data}) {
    ...
    useEffect(()=>{
        if(data[0]?.type_id){
            fetch(`${process.env.REACT_APP_HOST}/project/list?c=5&type_id=${data[0]?.type_id}&title=&p=0&lang=${currentLang || 'vi'}`)
            .then(response => response.text())
            .then(result => {
                const data = JSON.parse(result).result
                setProjectRelated(data.data)
            })
        ...
        <>
            <Typography sx={{marginBottom:theme.spacing(3)}} variant='h5' textAlign={"center"} fontWeight={700} color={theme.color.red}>{t('Dự án liên quan')}</Typography>
            {
                projectRelated.map((related)=>{
                    ...
                    return(
                        <Box key={'project-related'+related.id_project} sx={{paddingBottom: theme.spacing(3)}}>
                            <Link to={`/${t('duan')}/${normalizedTitle}.${related.id_project}`}>
